...
while lanjut:
    ...
    print("")
    if(p == 1):
        mycursor.execute("SELECT * FROM identitas")
        ...
        email = input("EMAIL : ")
        no_hp = input("NO HP : ")
        sql = "INSERT INTO identitas (nama, email, no_hp) VALUES (%s,%s,%s)"
        ...
        id = input("ID USER : ")
        mycursor.execute("SELECT * FROM identitas where id ="+id+" LIMIT 1 ")
        myresult = mycursor.fetchall()
        user = None
        for x in myresult:
            user = x
        if(user != None):
            ...
            sql = "UPDATE identitas SET nama=%s,email=%s,no_hp=%s WHERE id=%s"
